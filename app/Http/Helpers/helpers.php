...
function showAmount($amount, $decimal = 8, $separate = true, $exceptZeros = false, $currencyFormat = true)
{
    $separator = '';
    if ($separate) {
        $separator = ',';
...
function getImage($image, $size = null, $profile = false)
{
    $clean = '';
...
{
    $ad = Advertise::where('resolution', $size)->active()->inRandomOrder()->first();
    if (!empty($ad)) {
        return  '<a  target="_blank" href="' . $ad->redirect_url . '"><img src="' . getImage(getFilePath('ads') . '/' . $ad->image) . '" alt="image" class="w-100"></a>';
    } else {
        return false;
    }
}
...
function getRatingByUserProduct($userId, $productId)
{
