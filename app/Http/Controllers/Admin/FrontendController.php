...
class FrontendController extends Controller
{
        ...
        foreach ($request->except('_token', 'video') as $inputField => $val) {
            if ($inputField == 'has_image' && $imgJson) {
                foreach ($imgJson as $imgValKey => $imgJsonVal) {
                    $validationRule['image_input.'.$imgValKey] = ['nullable','image','max:20480', new FileTypeValidate(['jpg','jpeg','png'])];
                ...
                $validationRule['image_input'] = ['nullable', 'image', 'max:20480', new FileTypeValidate(['jpeg', 'jpg', 'png'])];
                continue;
            }
    ...
    public function frontendSeoUpdate(Request $request, $key,$id){
        $request->validate([
            'image'=>['nullable','image','max:20480', new FileTypeValidate(['jpeg', 'jpg', 'png'])]
        ]);
        $hasSeo = @getPageSections()->$key->element->seo;
        if (!$hasSeo) {
            abort(404);
