...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $order['all']        = Sell::groupBy('code')->get()->count();
        $order['processing'] = Sell::processing()->groupBy('code')->get()->count();
        $order['delivered']  = Sell::delivered()->groupBy('code')->get()->count();
        $order['rejected']   = Sell::rejected()->groupBy('code')->get()->count();

        $latestOrders = Sell::latest()->limit(6)->groupBy('code')->get();
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'order', 'latestOrders'));
    }
    ...
    public function depositReport(Request $request)
    {
        ...
        foreach ($dates as $date) {
            $data[] = [
                'created_on' => $date,
                'deposits' => getAmount($deposits->where('created_on', $date)->first()?->amount ?? 0, 8),
            ];
        }
